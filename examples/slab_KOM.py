...
def main():
    ...
    Fg = KOM(geo.mesh, f0, s0, n0, dt=0.01)
    ...
    disp_file = "results_disp_slab_KOM.bp"
    strain_file_init = Path("results_strain_slab_KOM_init.xdmf")
    strain_file_end = Path("results_strain_slab_KOM_end.xdmf")
    ...
    for i in range(2, N + 2):
        # Grow one day
        for ti in np.arange(0, 1.0, Fg.dt):
            Fg.apply_stimulus(u)
            solver.solve(u)
            print("J = ", dolfinx.fem.assemble_scalar(dolfinx.fem.form(J * dx)))
            print("Jg = ", dolfinx.fem.assemble_scalar(dolfinx.fem.form(Jg * dx)))
